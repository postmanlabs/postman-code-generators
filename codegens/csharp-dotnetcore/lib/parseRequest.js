...
function parseFormData (requestBody, trimFields, bodyMode) {
  ...
  return requestBody[requestBody.mode].reduce((body, data) => {
    ...
    if (data.type === 'file') {
      body += `requestContent.Add(new StreamContent(File.OpenRead("${sanitize(data.src, trimFields)}")), ` +
      ...
      switch (bodyMode) {
        case 'urlencoded':
          body += `formData.Add(new KeyValuePair<string, string>("${sanitize(data.key, trimFields)}", ` +
                  `"${sanitize(data.value, trimFields)}"));\n`;
          break;
        case 'formdata':
          body += `requestContent.Add(new StringContent("${sanitize(data.value, trimFields)}"), ` +
                  `"${sanitize(data.key, trimFields)}");\n`;
          break;
...
function parseBody (request, trimFields) {
  ...
  if (!_.isEmpty(requestBody)) {
    switch (requestBody.mode) {
      ...
      case 'raw':
        return `request.Content = new StringContent(${JSON.stringify(requestBody[requestBody.mode])}, ` +
