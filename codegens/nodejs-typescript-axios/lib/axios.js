...
function makeSnippet (request, indentString, options) {
  ...
  snippet += 'import axios,{AxiosError, AxiosRequestConfig, AxiosResponse} from \'axios\';\n';
  ...
  snippet += ' config:AxiosRequestConfig = {\n';
  snippet += configArray.join(',\n') + '\n';
  snippet += '};\n\n';
  snippet += 'axios(config)\n';
  if (options.ES6_enabled) {
    snippet += '.then((response:AxiosResponse) => {\n';
  }
  else {
    snippet += '.then(function (response:AxiosResponse) {\n';
  }
  snippet += indentString + 'console.log(JSON.stringify(response.data));\n';
  snippet += '})\n';
  if (options.ES6_enabled) {
    snippet += '.catch((error:AxiosError) => {\n';
  }
  else {
    snippet += '.catch(function (error:AxiosError) {\n';
  }
