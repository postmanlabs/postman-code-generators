...
/**
 * Parses body object based on mode of body and converts into nodejs(superagent) code snippet
 *
    ...
    _.forEach(Object.keys(headerObject), (key) => {
      if (Array.isArray(headerObject[key])) {
        _.forEach(headerObject[key], (value) => {
          headerSnippet += indentString + `.set('${sanitize(key, true)}', '${sanitize(value)}')\n`;
        });
      }
      else {
        headerSnippet += indentString + `.set('${sanitize(key, true)}', '${sanitize(headerObject[key])}')\n`;
      }
    });
  }
  console.log({headerSnippet});
