...
self = module.exports = {
  ...
  * @param  {Fun  ction} callback - function with parameters (error, snippet)
  */
  ...
  convert: function (request, options, callback) {
    ...
    if (_.isFunction(options)) {
      ...
      throw new Error('dart-http~convert: Callback is not a function');
    }
    options = sanitizeOptions(options, self.getOptions());
    indentation = identity.repeat(options.indentCount);
    snippet += 'import \'dart:convert\' as convert;\n';
    snippet += 'import \'package:http/http.dart\' as http;\n\n';
    snippet += 'void main() async {\n';
    snippet += `const url = "${sanitize(request.url.toString(), 'url')}";\n`;
    ...
    snippet += `var response = await http.${request.method.toLowerCase()}(url,\n\theaders: headers,\n`;
    if (request.method.toLowerCase() !== 'delete') {
      snippet += request.body && request.body.mode && request.body.mode === 'formdata' ?
        '\tbody: payload, files: files' : request.method.toLowerCase() === 'get' ? '' : '\tbody:payload\n';
    }
    snippet += ');\n\n';
    snippet += 'print(response.body);';
    snippet += '\n}';
