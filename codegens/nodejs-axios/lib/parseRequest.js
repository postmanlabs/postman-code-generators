...
/**
 * parses body of request when type of the request body is formdata
 * returns code snippet for nodejs to add body
 *
 * @param {Array<Object>} dataArray - array containing body elements of request
 * @param {String} indentString - string required for indentation
 * @param {Boolean} trimBody - indicates whether to trim body or not
 */
function constructFormDataObject(dataArray, indentString, trimBody) {
  if (!dataArray) {
    return '';
  }
  
  let snippetString = 'const formData = new FormData();\n'
  const itemsArray = _.reduce(dataArray, (accumalator, item) => {
    if (item.disabled) {
      return accumalator;
    }
    /* istanbul ignore next */
    if (item.type === 'file') {
      accumalator.push(
        `formData.append('${sanitize(item.key, trimBody)}', '${sanitize(item.value, trimBody)}', '${sanitize(item.src, trimBody)}');`
        );
    }
    else {
      accumalator.push(
        `formData.append('${sanitize(item.key, trimBody)}', '${sanitize(item.value, trimBody)}');`
      );
    }
    return accumalator;
  }, []);
  return snippetString += itemsArray.join('\n');
}
/**
...
function parseBody (requestbody, indentString, trimBody) {
  if (requestbody) {
    switch (requestbody.mode) {
      case 'raw':
        return `data: ${JSON.stringify(requestbody[requestbody.mode])}\n`;
      // case 'formdata':
...
module.exports = {
  ...
  constructFormDataObject : constructFormDataObject
};
