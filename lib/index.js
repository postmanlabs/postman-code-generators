...
module.exports = {
   ...
   *
   * @param {Object} language - language key provided by getLanguageList function
   * @param {string} variant - variant key provided by getLanguageList function
   * @param {String} request -  valid postman request
   * @param {{trimRequestBody: boolean, indentCount: number, indentType: string, followRedirect: boolean}} [options] - contains convert level options
  ...
  convert (language, variant, request, options, callback) {
    ...
    languageMap.forEach((codegen) => {
      ...
      var lang = codegen.lang.trim(),
        currentVariant = codegen.variant.trim();
      // console.log('language = ' + language + ' lang = ' + lang + ' variant = ' + variant + ' currentVariant = ' + currentVariant)
