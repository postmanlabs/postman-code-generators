  ...
  getOptions (language, variant, callback) {
    var validCodegen = languageMap.filter((codegen) => {
      ...
      return (
        ...
        variant.toLowerCase() === currentVariant.toLowerCase()
      );
    });
    try {
      validCodegen.forEach((codegen) => {
        ...
        if (typeof main.getOptions !== 'function') {
          throw 'Codegen~getOptions: getOptions is not a function';
        }
        if (!main.getOptions) {
          throw 'Codegen~convert: Could not find condegen corresponding to provided language, variant pair';
        }
  ...
  convert (language, variant, request, options, callback) {
    ...
    try {
      if (!sdk.Request.isRequest(request)) {
        throw 'Codegen~convert: Invalid request';
      }
      ...
      languageMap.forEach((codegen) => {
        ...
        if (
          ...
          if (typeof convert !== 'function') {
            throw 'Codegen~convert: Convert is not a function';
          }
        }
      });
      if (!convert) {
