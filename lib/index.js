...
module.exports = {
  ...
  getOptions (language, variant, callback) {
    ...
    validCodegen.forEach((codegen) => {
      ...
      if (typeof main.getOptions !== 'function') {
        throw new Error('Codegen~getOptions: getOptions is not a function');
      }
      if (!main.getOptions) {
        throw new Error('Codegen~convert: Could not find condegen corresponding to provided language, variant pair');
      }
  ...
  convert (language, variant, request, options, callback) {
    ...
    if (!sdk.Request.isRequest(request)) {
      throw new Error('Codegen~convert: Invalid request');
    }
    ...
    languageMap.forEach((codegen) => {
      ...
      if (language.toLowerCase() === lang.toLowerCase() && variant.toLowerCase() === currentVariant.toLowerCase()) {
        ...
        if (typeof convert !== 'function') {
          throw new Error('Codegen~convert: Convert is not a function');
        }
      }
    });
    if (!convert) {
